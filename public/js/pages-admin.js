...
App.page('messages', {
  ...
  help: [
    ...
    '官方帳號的說明卡片對同一個未綁定的人 24 小時內只會發一次，不會每句話都回一張。',
      ...
      b.onclick = async () => {
        ...
        m.body.querySelector('#send').onclick = async () => {
          ...
          try {
            ...
            // status 'manual' 是所方還沒設定 LINE 權杖，不是對方沒綁定，兩者要分清楚
            else if (r.line && r.line.status === 'manual') UI.toast('已存入系統（尚未設定 LINE 權杖，需人工通知）');
